...
#define LightName       @"light"
#define EyeName         @"eye"
#define ContainerName	@"container"


@implementation _D_viewer_3Scene

-(void) dealloc {
	[super dealloc];
}

-(void) loadPodfile:(NSString*)fileName {
    CC3PODResourceNode* node = (CC3PODResourceNode*)[self getNodeNamed:ObjectName];
    if (node != Nil)
        [self removeChild:node];
    [self addContentFromPODFile: fileName withName:ObjectName];
}
...
-(void) initSceneComponent {
	...
	self->cam = [CC3Camera nodeWithName:CameraName];
	self->light = [CC3Light nodeWithName:LightName];
    self->eye = [CC3Node nodeWithName:EyeName];
    self->container = [CC3Node nodeWithName:ContainerName];
...
-(void) getMaxWidth {
     ...
     */
    self->maxDepth = MAX(MAX(self->Dobj.boundingBox.maximum.z - self->Dobj.boundingBox.minimum.z,
...
-(void) initializeScene {
	...
	[self loadPodfile:@"oilDrum-anim-noinvert-tryColor.pod"];
